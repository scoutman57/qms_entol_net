...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		if(get_cookie('sma_language')) {
			if(file_exists('invoice/language/'.get_cookie('sma_language').'/sma_lang.php') && is_dir('invoice/language/'.get_cookie('sma_language'))) {
		...
		//define("ALERT_NO", $this->ion_auth_model->get_total_results());
			...
			foreach ($user_groups as $user_group ){
				define("GROUPID", $user_group['id']);
				define("GROUPNAME", $user_group['name']);
				define("GROUPDESC", $user_group['description']);
			}
